...
function CustomTooltip({ active, payload }) {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[#18181b] border border-[#27272a] rounded-lg px-3 py-2 shadow-2xl text-xs space-y-1">
    ...
    <div className="flex-1 bg-[#121214] text-[#EDEDED] font-sans antialiased min-h-[calc(100vh-3.5rem)] flex flex-col p-4 sm:p-6 lg:p-8 space-y-6 select-none overflow-y-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#222226]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#1c1c1f] border border-[#2e2e33] text-xs text-[#d1d5db]">
            ...
            <button onClick={() => toast.info("Project: Default Workspace")} className="text-[#9ca3af] hover:text-white ml-0.5 cursor-pointer">x</button>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={handleRefresh} className={`p-2 rounded-lg bg-[#1c1c1f] hover:bg-[#25252a] border border-[#2e2e33] text-[#9ca3af] hover:text-white transition-all cursor-pointer ${isRefreshing ? "animate-spin text-[#38BDF8]" : ""}`}>
            <RotateCcw className="w-3.5 h-3.5" />
          </button>
          <button onClick={handleExport} className="p-2 rounded-lg bg-[#1c1c1f] hover:bg-[#25252a] border border-[#2e2e33] text-[#9ca3af] hover:text-white transition-all cursor-pointer">
      ...
      <div className="pt-8 space-y-8">
        ...
        <div className="flex flex-col items-center text-center gap-3">
          <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#18181b] border border-[#27272a] text-[#a1a1aa]">
            <Star className="w-3 h-3 text-[#f59e0b] fill-[#f59e0b]" />
            <span className="font-bold uppercase text-[10.5px] tracking-wide">Subscription Plans</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-stretch">
          {PLANS.map((plan) => {
            ...
            return (
              <div key={plan.id} className={`relative flex flex-col rounded-2xl border ${plan.border} p-6 gap-5 transition-all duration-300 hover:shadow-2xl ${
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center gap-2.5">
                    <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: `${plan.color}15`, border: `1px solid ${plan.color}28` }}>
                      <PlanIcon className="w-5 h-5" style={{ color: plan.color }} />
                    </div>
                    <div>
                      <div className="text-[15px] font-bold text-white leading-tight">{plan.name}</div>
                ...
                {/* CTA */}
                <button onClick={() => plan.current ? toast.info("You are on the Free plan") : plan.id === "enterprise" ? toast.info("Contact sales@threatlens.io") : toast.success(`Upgrading to ${plan.name}...`)}
