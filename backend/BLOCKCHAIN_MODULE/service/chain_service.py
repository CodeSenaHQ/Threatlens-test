...
class InternalChain:
    def __init__(self, chain_name: str, user: dict):
        ...
        self.chain_dir = (
            Path(blockchain_dir) / "chains" / str(self.account_id)
        )
        self.chain_file = self.chain_dir / f"{self.chain_id}.json"
    ...
    def create_block(self, type: str, data: dict | list[dict]):
        if isinstance(data, dict):
            return self._create_single_block(type, data)

        if isinstance(data, list):
            if not all(isinstance(item, dict) for item in data):
                raise TypeError("data must be a dict or a list of dictionaries")
            return [self._create_single_block(type, item) for item in data]

        raise TypeError("data must be a dict or a list of dictionaries")

    def commit(self):
        self.chain_dir.mkdir(parents=True, exist_ok=True)
        with self.chain_file.open("w", encoding="utf-8") as file:
            json.dump(self.chain, file, ensure_ascii=False, indent=2)

    def load_chain(self, page: int = 1, limit: int = 10) -> list[dict]:
        self._validate_pagination(page, limit)

        non_genesis = self.chain[1:]
        end = len(non_genesis) - ((page - 1) * limit)
        start = max(0, end - limit)
        selected = non_genesis[start:end]

        return [self.get_genesis(), *selected]
    ...
    def get_block(self, idx: int) -> dict:
        for block in self.chain:
            if block["index"] == idx:
                return block
        raise IndexError(f"Block with index {idx} does not exist")
    ...
    def verify_chain(
        ...
        if mode == "single":
            ...
            return self._verify_range(
                target, target, include_predecessor=True
            )

        if mode == "from":
            if target < 0 or target >= len(self.chain):
                raise IndexError("target is outside the chain")
            return self._verify_range(target, len(self.chain) - 1)

        if mode == "till":
            if target < 0 or target >= len(self.chain):
                raise IndexError("target is outside the chain")
            return self._verify_range(0, target)

        if mode == "full":
            return self._verify_range(0, len(self.chain) - 1)
        ...
        if mode == "last":
            ...
            start = max(1, end - target + 1)

            return self._verify_range(
                start, end, include_predecessor=start > 0
            )
        ...
        if mode == "latest":
            ...
            if latest == 0:
                return self._verify_block(self.chain[0])

            return self._verify_range(
                latest, latest, include_predecessor=True
            )

        raise ValueError(
            "mode must be one of: single, from, till, full, last, latest"
        )
    ...
    def _create_single_block(self, type: str, data: dict) -> dict:
    ...
    def _build_block(
        ...
        current = self._calculate_hash(block_without_current)
    ...
    def _load_from_file(self):
        with self.chain_file.open("r", encoding="utf-8") as file:
            loaded = json.load(file)

        if not isinstance(loaded, list):
            raise ValueError("Chain JSON must contain a list of blocks")
    ...
    def _verify_range(
        ...
        if include_predecessor and start > 0:
            verification_start = start - 1

        for position in range(verification_start, end + 1):
            ...
            if block["index"] != previous["index"] + 1:
                return {
                    "status": False,
                    "message": "Block index sequence is invalid",
            ...
            if block["prev"] != previous["current"]:
                return {
                    "status": False,
                    "message": "Previous block hash linkage failed",
                    "failure_type": "previous_hash",
                    "block_index": block["index"],
                }
    ...
    def _verify_block(self, block: dict) -> dict:
        block_without_current = {
            "index": block["index"],
            "type": block["type"],
            "data": block["data"],
        ...
        expected_hash = self._calculate_hash(block_without_current)
